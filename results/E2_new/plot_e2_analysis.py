...
def plot_training_consistency(runs, summary, results_dir):
    """Plot 4: Training time consistency and stability"""
    fig, ax = plt.subplots(figsize=(12, 7))
    
    time_by_batch = {}
    for run in runs:
        bs = run['batch_size']
        if bs not in time_by_batch:
            time_by_batch[bs] = []
        time_by_batch[bs].append(run['train_time_s'])
    
    batch_sizes = sorted(time_by_batch.keys())
    
    # Create box plot + strip plot
    data_to_plot = [time_by_batch[bs] for bs in batch_sizes]
    positions = np.arange(len(batch_sizes))

    bars = ax.bar(positions, [np.mean(vals) for vals in data_to_plot], width=0.55,
                  color='#3498db', alpha=0.75, edgecolor='#2c3e50', linewidth=1.5)
    
    # Overlay individual points
    for i, bs in enumerate(batch_sizes):
        y = time_by_batch[bs]
        x = np.random.normal(i, 0.04, size=len(y))
        ax.scatter(x, y, alpha=0.6, s=100, color='#2c3e50', edgecolors='black', linewidth=1)
    
    ax.set_xlabel('Batch Size', fontsize=12, fontweight='bold')
    ax.set_ylabel('Training Time (seconds)', fontsize=12, fontweight='bold')
    ax.set_title('E2: Training Time Consistency Across 3 Runs', fontsize=14, fontweight='bold', pad=20)
    ...
    for i, bs in enumerate(batch_sizes):
        mean_time = np.mean(time_by_batch[bs])
        ax.text(i, mean_time + 0.3, f'{mean_time:.2f}s', ha='center', va='bottom',
               fontsize=10, fontweight='bold')
    
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, "plots", "04_training_consistency.png"),
                dpi=300, bbox_inches='tight', facecolor='white')
    print("✓ 04_training_consistency.png")
    plt.close()

def plot_comprehensive_summary(runs, summary, results_dir):
    """Plot 5: 4-panel comprehensive summary"""
    ...
    overheads = [summary[bs]['overhead_pct'] for bs in batch_sizes]
    colors = ['#2ecc71' if o < 5 else '#e74c3c' for o in overheads]
    bars = ax1.bar(range(len(batch_sizes)), overheads, alpha=0.85, color=colors, 
                   edgecolor='#2c3e50', linewidth=1.5)
    ax1.axhline(y=5.0, color='#e74c3c', linestyle='--', linewidth=2, alpha=0.7)
    ax1.set_ylabel('Overhead (%)', fontweight='bold')
    ax1.set_title('CodeCarbon Overhead', fontweight='bold', fontsize=12)
    ...
    energy_means = [np.mean(energy_by_batch[bs]) for bs in batch_sizes]
    bars = ax2.bar(range(len(batch_sizes)), energy_means, alpha=0.85, 
                   color=['#3498db', '#2980b9', '#1f618d'], edgecolor='#2c3e50', linewidth=1.5)
    ax2.set_ylabel('GPU Energy (kWh)', fontweight='bold')
    ax2.set_title('Total GPU Energy', fontweight='bold', fontsize=12)
    ...
    power_means = [np.mean(power_by_batch[bs]) for bs in batch_sizes]
    bars = ax3.bar(range(len(batch_sizes)), power_means, alpha=0.85, 
                   color=['#e74c3c', '#c0392b', '#a93226'], edgecolor='#2c3e50', linewidth=1.5)
    ax3.set_ylabel('Avg Power (W)', fontweight='bold')
    ax3.set_title('Average GPU Power', fontweight='bold', fontsize=12)
    ax3.set_xticks(range(len(batch_sizes)))
    ax3.set_xticklabels([f'BS={bs}' for bs in batch_sizes])
    ax3.grid(axis='y', alpha=0.2, linestyle=':')
    
    # Panel 4: Steps and throughput
    ax4 = fig.add_subplot(gs[1, 1])
    steps_by_batch = {}
    time_by_batch = {}
    for run in runs:
        bs = run['batch_size']
        if bs not in steps_by_batch:
            steps_by_batch[bs] = []
            time_by_batch[bs] = []
        steps_by_batch[bs].append(run['steps'])
        time_by_batch[bs].append(run['train_time_s'])
    
    throughput = []
    for bs in batch_sizes:
        avg_steps = np.mean(steps_by_batch[bs])
        avg_time = np.mean(time_by_batch[bs])
        throughput.append((avg_steps * bs) / avg_time)  # samples/sec
    
    bars = ax4.bar(range(len(batch_sizes)), throughput, alpha=0.85,
                   color=['#9b59b6', '#8e44ad', '#7d3c98'], edgecolor='#2c3e50', linewidth=1.5)
    ax4.set_ylabel('Throughput (samples/sec)', fontweight='bold')
    ax4.set_title('Training Throughput', fontweight='bold', fontsize=12)
    ax4.set_xticks(range(len(batch_sizes)))
    ax4.set_xticklabels([f'BS={bs}' for bs in batch_sizes])
    ax4.grid(axis='y', alpha=0.2, linestyle=':')
    
    plt.suptitle('E2: Comprehensive Energy & Performance Summary', 
                 fontsize=16, fontweight='bold', y=0.995)
    
    plt.savefig(os.path.join(results_dir, "plots", "05_comprehensive_summary.png"),
                dpi=300, bbox_inches='tight', facecolor='white')
...
def main():
    ...
    plot_training_consistency(runs, summary, results_dir)
    plot_comprehensive_summary(runs, summary, results_dir)
